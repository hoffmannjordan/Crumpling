...
voronoicell_base_2d::voronoicell_base_2d() :
	current_vertices(init_vertices), ed(new int[2*current_vertices]),
	pts(new double[2*current_vertices]) {
}

/** Constructs a 2D Voronoi cell class by copying contents from another.
 * \param[in] cp a point to the other class. */
voronoicell_base_2d::voronoicell_base_2d(voronoicell_base_2d *cp) :
	current_vertices(cp->current_vertices), p(cp->p),
	ed(new int[2*current_vertices]), pts(new double[2*current_vertices]) {
	for(int i=0;i<2*p;i++) ed[i]=cp->ed[i];
	for(int i=0;i<2*p;i++) pts[i]=cp->pts[i];
}
...
bool voronoicell_base_2d::nplane_cut(vc_class &vc,double x,double y,double rsq,int p_id,double u,int up) {
	...
	for(unsigned int i=0;i<ds.size();i++) ed[ds[i]*2]=-1;
...
voronoicell_neighbor_2d::voronoicell_neighbor_2d(voronoicell_neighbor_2d &c_) :
	...
	for(int i=0;i<p;i++) ne[i]=c_.ne[i];
}
